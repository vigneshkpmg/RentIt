...
class mediaController {
    ...
    public async saveMedia(req: any, res: any): Promise<any> {
        try {       
            ...
            return res.status(201).send({ data: { file: results }, container:containerName });
        }
    ...
    public async GetMeadia(req:any, res:any) {
        ...
        try {
            ...
            // Get a reference to a container
            const containerClient = blobServiceClient.getContainerClient(containerName);
            const blobList = containerClient.listBlobsFlat().byPage({ maxPageSize: 5 });
            let results: string[] = [];
            for await (const blob of blobList) {
                for (const item of blob.segment.blobItems) {
                    ...
                    results.push((await streamToBuffer(downloadBlockBlobResponse.readableStreamBody!)).toString("base64"));
