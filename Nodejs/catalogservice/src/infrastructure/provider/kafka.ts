...
class KafkaProvider{
    ...
    */
    constructor() {
        this.schemaRegistry= new SchemaRegistry({host:environment.config().schemaRegistryUrl}) 
    ...
    async CreateKafkaTopicAndRegisterSchema(key:any,schema: any, topic: string) {
          const requestValSchema = {
            url: `${environment.config().schemaRegistryUrl}/subjects/${topic}/versions`,
            method: 'post',
